...
JumpPointSearch(int map_size = 20, int max_iter = 100);
~JumpPointSearch();
...
void Jump_Point_Search(int start[], int target[]);
 ...
 JumpPointSearch::JumpPointSearch(int map_size, int max_iter)
{ogm_size = map_size;
...
int JumpPointSearch::check_search(int x, int y, int z)
	...
	void JumpPointSearch::Jump_Point_Search(int start[], int target[])
	{
		...
		while (iterations < max_iterations && (x_node != x_dest || y_node != y_dest || z_node != z_dest))
		{
			...
			if (index == -1)
			{
				cout << "\nPath does not exist! Potential nodes exhausted! Iterations: "<<iterations<<"Nodes added to queue: "<<node_array.size()<<endl;
				return;
			}
		...
		{
			cout << "\nPath does not exist! Iteration limit reached! Iterations: "<<iterations<<endl;
		}
	}


	
void find_free_node(int ***ogm,int &x, int &y, int &z, int max_scope, int map_size)
{
 if (ogm[x][y][z] == 1)
 {return;
 }
...
int main()
{
  //Eigen::Quaterniond q(0.707, 0, 0.707, 0);
 ...
 // m(1,1) = m(1,0) + m(0,1);
  ...
  //std::cout << m << std::endl;
  //path_planning::JumpPointSearch JPS(40,150);
  JumpPointSearch JPS(100,1000);
  //for (int i = 1; i<100;i++)
  //for (int j = 1; j<100;j++)
  //for (int k = 1; k<100;k++)
  //JPS.ogm[i][j][k] = 0;
  ...
  //int dest_y = 50;
  //int dest_z = 50;
  //find_free_node(JPS.ogm,dest_x,dest_y,dest_z,100,100);
  //std::cout<<"Destination:"<<dest_x<<dest_y<<dest_z<<std::endl;
  //std::cout<<JPS.ogm[dest_x][dest_y][dest_z]<<std::endl;
  bool is = false;
  if(is==false)
  {std::cout<<"Yes\n";
   is=true;}
  ...
  JPS.ogm[50][i][j] = 0;
  int start[3] = {0,0,3};
  int target[3] = {99,99,99};
  JPS.Jump_Point_Search(start,target);
  std::cout<<"Start:"<<start[0]<<start[1]<<start[2]<<std::endl;
  std::cout<<"Destination:"<<target[0]<<target[1]<<target[2]<<std::endl;
  std::cout<<"Node Array Size: "<<JPS.node_array.size()<<std::endl;
  for (int k = JPS.rpath.size() - 1; k>=0;k--)
  {
  	std::cout<<"x: "<<JPS.rpath[k].current_node_x;
	std::cout<<",y: "<<JPS.rpath[k].current_node_y;
	std::cout<<",z :"<<JPS.rpath[k].current_node_z;
	std::cout<<",Distance :"<<JPS.rpath[k].dist_start;
	std::cout<<std::endl;
  }
  //std::cout<<"Distance :"<<JPS.rpath[0].dist_start<<std::endl;
  std::cout<<"Success!!"<<std::endl;
  //std::cout<<JPS.check_search(39,39,29,1)<<std::endl;
  //std::cout<<JPS.node_array.size()<<std::endl;
}
